<template>
  ...
  <div v-if="load" style='text-align:center;' class="content">
    ...
    <select  v-model="dataEntry.date" class="form-control" id="exampleFormControlSelect2">
      <option v-for="vn in vents" :key="vn.idpv" :value="vn.date"> {{ vn.date }} </option>
    </select>
  </div>
...
export default {
    ...
    methods : {
      ...
      async generateVente()
      {
          const res = await this.callApi('POST','data/getAllPv',this.dataEntry)
          if(res.status === 200)
          {
              this.load = true
            this.vents = res.data
      ...
      async deleteVente()
      {
          const res = await this.callApi('POST','data/deletePvByVendeur',this.dataEntry)
          if(res.status === 200)
          {
