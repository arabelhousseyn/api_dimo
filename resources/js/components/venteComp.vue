<template>
  ...
  <div v-if="load" style='text-align:center;' class="content">
    <div class="form-group">
    <label for="exampleFormControlSelect2" style='font-weight:bold;'>Date de ventes</label>
    ...
    <button @click="deleteVente" class='btn btn-danger'>Supprimer La vente</button>
  </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods : {
      ...
      async generateVente()
      {
          const res = await this.callApi('POST','data/AllventesByVendeur',this.dataEntry)
          if(res.status === 200)
          {
              this.load = true
            this.vents = res.data
      ...
      async deleteVente()
      {
          const res = await this.callApi('POST','data/deleteVeneteByDate',this.dataEntry)
          if(res.status === 200)
          {
